...
if(isset($_GET['email_confirmation'])) {
    ...
    $query = select("SELECT * FROM `user_crud` WHERE `email`=? AND `token`=? LIMIT 1", [$data['email'], $data['token']], 'ss');
    ...
    if(mysqli_num_rows($query) == 1) {
        ...
        if($fetch['is_verified'] == 1) {
            ...
            $update = update("UPDATE `user_crud` SET `is_verified`=? WHERE `id`=?", [1, $fetch['id']], 'ii');
